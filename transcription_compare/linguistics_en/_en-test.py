# Put the linguistics_en library in the same folder as your script so NodeBox can find
# the library. It takes some time to load all the data the first time.
try:
    # This is the statement you normally use.
    en = ximport("linguistics_en")
except:
...
# linguistics_en.list.conjunction(linguistics_en, generalize=True) ->
# a number of modules, a number of functions, a number of strings, 
# a pair of lists, a pair of dictionaries, an linguistics_en verb, an linguistics_en sentence,
# an linguistics_en number, an linguistics_en noun, an linguistics_en list, an linguistics_en content, an linguistics_en adverb,
# an linguistics_en adjective, a None type and a DrawingPrimitives Context
...
# wolf -> wolves
# child -> children
# You can also do linguistics_en.adjective.plural().
...
# You can use the linguistics_en.list.flatten() command to flatten the list:
print 33, en.list.flatten(en.noun.senses("tree"))
# -> ['tree', 'tree', 'tree diagram', 'Tree', 'Sir Herbert Beerbohm Tree']

# If you want a list of all nouns/verbs/adjectives/adverbs there's the
# linguistics_en.wordnet.all_nouns(), linguistics_en.wordnet.all_verbs() ... commands:
...
# linguistics_en.verbs.hypernyms("run"), linguistics_en.adjective.gloss("beautiful") etc. are valid commands.
...
# Assuming you would want to summarise web content you can use linguistics_en.content.strip_tags()
